...
namespace Atos.DBViewer
{
    public partial class DBViewer : Form
    {
        ...
        static List<Message> PlotList = new List<Message>();
        ...
        public DBViewer()
        {
            ...
            //chtPlot.ChartAreas[0].AxisY.Maximum = 70;
            //chtPlot.ChartAreas[0].AxisY.Minimum = 0;
            //chtPlot.ChartAreas[0].AxisX.Minimum = 0;
        ...
        public static List<Message> GetPlot(int tsk)
        {
            var plot = msgList.Where(
                    m =>
                        m.PayloadTargetId == 0 && m.PayloadType == "s" &&
                        m.PayloadTaskNumber == tsk)
                .OrderByDescending(m => m.SequenceNumber);
            
            return plot.Take(100).ToList();
        }
        ...
        private void btnPlot_Click(object sender, EventArgs e)
        {
            ...
            chtPlot.Visible = !chtPlot.Visible;
            lblLeftError.Visible = chtPlot.Visible;
            lblRightError.Visible = chtPlot.Visible;
        }

        private void AutoPlot()
        {
            chtPlot.Series["leftPid"].Points.Clear();
            chtPlot.Series["rightPid"].Points.Clear();
            chtPlot.Series["leftSpeed"].Points.Clear();
            chtPlot.Series["rightSpeed"].Points.Clear();
            PlotList.Clear();
            PlotList = GetPlot(Convert.ToInt32(numTsk.Value));
            for (int i = 0; i < PlotList.Count; i++)
            {
                chtPlot.Series["leftPid"].Points.AddXY(i, PlotList[i].PayloadArg0);
                chtPlot.Series["rightPid"].Points.AddXY(i, PlotList[i].PayloadArg1);
                chtPlot.Series["leftSpeed"].Points.AddXY(i, PlotList[i].PayloadArg2);
                chtPlot.Series["rightSpeed"].Points.AddXY(i, PlotList[i].PayloadArg3);
            }

            lblLeftError.Text = "Total Left Error: " + (PlotList.Select(m=>m.PayloadArg0).Sum() - PlotList.Select(m => m.PayloadArg2).Sum());
            lblRightError.Text = "Total Right Error: " + (PlotList.Select(m => m.PayloadArg1).Sum() - PlotList.Select(m => m.PayloadArg3).Sum());
        }

        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlArg.SelectedIndex >= 0)
            {
